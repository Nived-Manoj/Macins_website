...
import { useState } from 'react';
import { MapPin, Phone, Mail, Send, Globe, Check, Sparkles } from 'lucide-react';
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative py-24 bg-gradient-to-br from-teal-700 via-teal-600 to-cyan-600 overflow-hidden min-h-screen">
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-white rounded-full mix-blend-overlay filter blur-3xl opacity-10 animate-pulse" style={{ animationDelay: '1s' }}></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-cyan-300 rounded-full mix-blend-overlay filter blur-3xl opacity-10 animate-pulse" style={{ animationDelay: '2s' }}></div>
      </div>
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center space-x-2 bg-white/20 backdrop-blur-sm border border-white/30 text-white px-5 py-2.5 rounded-full text-sm font-semibold mb-6 shadow-lg">
            <Sparkles size={16} />
            <span>Get in Touch</span>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
            <div className="grid lg:grid-cols-5 gap-0">
              ...
              <div className="lg:col-span-3 p-8 lg:p-12 bg-white">
                {submitted ? (
                  <div className="flex flex-col items-center justify-center h-full space-y-4">
                    <div className="w-24 h-24 bg-gradient-to-br from-teal-600 to-cyan-600 rounded-full flex items-center justify-center animate-pulse">
                      <Check size={48} className="text-white" />
                    </div>
                    <h3 className="text-3xl font-bold text-gray-900">Message Sent!</h3>
                    <p className="text-gray-600 text-center text-lg">Thank you for your interest. We will contact you shortly.</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                    <div>
                    ...
                    <div className="grid md:grid-cols-2 gap-5">
                      <div className="group">
                        ...
                        <input
                          ...
                          value={formData.name}
                          onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                          className="w-full px-4 py-3.5 rounded-xl border-2 border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400 focus:border-teal-600 focus:ring-4 focus:ring-teal-100 outline-none transition-all focus:bg-white"
                          placeholder="John Doe"
                        />
                      </div>
                      <div className="group">
                    ...
                    <div className="grid md:grid-cols-2 gap-5">
                      ...
                      <div className="group">
                        ...
                        <select
                          value={formData.service}
                          onChange={(e) => setFormData({ ...formData, service: e.target.value })}
                          className="w-full px-4 py-3.5 rounded-xl border-2 border-gray-200 bg-gray-50 text-gray-900 focus:border-teal-600 focus:ring-4 focus:ring-teal-100 outline-none transition-all focus:bg-white"
                        >
                          <option value="">Select a service</option>
                          <option value="mep">MEP Contracting</option>
                          <option value="interior">Interior Fit-Out</option>
                    ...
                    <div className="group">
                      ...
                      <textarea
                        ...
                        value={formData.message}
                        onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                        className="w-full px-4 py-3.5 rounded-xl border-2 border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400 focus:border-teal-600 focus:ring-4 focus:ring-teal-100 outline-none transition-all resize-none focus:bg-white"
                        placeholder="Tell us about your project requirements, timeline, and budget..."
                      />
                    </div>
                    ...
                    <button
                      onClick={handleSubmit}
                      disabled={isSubmitting}
                      className="w-full bg-gradient-to-r from-teal-600 to-cyan-600 text-white px-8 py-4 rounded-xl hover:shadow-2xl hover:shadow-teal-600/50 transition-all duration-300 font-bold flex items-center justify-center space-x-3 disabled:opacity-50 disabled:cursor-not-allowed group transform hover:scale-105 active:scale-95"
                    >
                      <span className="text-lg">{isSubmitting ? 'Sending...' : 'Send Message'}</span>
                      <Send size={20} className="group-hover:translate-x-1 transition-transform" />
                    </button>
              ...
              <div className="lg:col-span-2 p-8 lg:p-10 bg-white/10 backdrop-blur-sm border-l border-white/20">
                <div className="flex items-center space-x-3 mb-8">
                ...
                <div className="space-y-5">
                  {offices.map((office, index) => (
                    <div 
                      key={index} 
                      className="group relative bg-white/90 backdrop-blur-sm border border-white rounded-2xl p-5 transform transition-all duration-300 hover:scale-105 hover:bg-white hover:shadow-xl cursor-pointer overflow-hidden"
                    >
                      {/* Gradient overlay on hover */}
                      <div className={`absolute inset-0 bg-gradient-to-br ${office.gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-300 rounded-2xl`}></div>
                      
                      <div className="relative z-10">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <h4 className="font-bold text-lg text-gray-900 mb-0.5">{office.city}</h4>
                            <p className="text-sm text-gray-600">{office.country}</p>
                          </div>
                          <div className={`w-8 h-8 bg-gradient-to-br ${office.gradient} rounded-lg flex items-center justify-center opacity-60 group-hover:opacity-100 transition-opacity`}>
                        ...
                        <div className="space-y-2.5 text-sm">
                          ...
                          <div className="flex items-center space-x-2.5">
                            ...
                            <a href={`tel:${office.phone}`} className="text-gray-700 hover:text-teal-600 transition-colors">
                              {office.phone}
                            </a>
                          </div>
                          <div className="flex items-center space-x-2.5">
                            <Mail size={16} className="flex-shrink-0 text-teal-600" />
                            <a href={`mailto:${office.email}`} className="text-gray-700 hover:text-teal-600 transition-colors">
                              {office.email}
                            </a>
                          </div>
                        </div>
                ...
                {/* Additional Info */}
                <div className="mt-8 p-5 bg-white/90 backdrop-blur-sm border border-white rounded-2xl">
                  <p className="text-sm text-gray-700 text-center">
                    <span className="font-bold text-teal-600">Available 24/7</span> for emergency support and consultations
                  </p>
                </div>
              </div>
            </div>
